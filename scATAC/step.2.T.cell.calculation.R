...
tempdf <- read_tsv('/gpfs/output/20190304_PD1/DML/forMerge.allcomparison.bed',col_names = c('chr','start','end','score'),col_types = cols(col_character(),col_integer(),col_integer(),col_double()))
tempdf$chr <- paste0('chr',tempdf$chr)
AnnotationSet[['CD4CD8PD1DML']] <- reduce(flank(easyLift::easyLiftOver(makeGRangesFromDataFrame(tempdf),map='hg19_hg38'),width=200))

loadArchRProject(path='Save/Bladder_T_cell_20201204/') -> proj_subtype


proj_subtype <- addIterativeLSI( ArchRProj = proj_subtype,useMatrix = "TileMatrix", name = "IterativeLSI", iterations = 4,clusterParams = list( resolution = c(0.1, 0.2, 0.4,1), sampleCells = 10000, n.start = 10), varFeatures = 50000, dimsToUse = 1:30)
# Add cluster based on LSI
proj_subtype <- addClusters(input = proj_subtype,reducedDims = "IterativeLSI",method = "Seurat",name = "seurat_clusters",resolution = 1.5)
proj_subtype <- addImputeWeights(proj_subtype,reducedDims = "IterativeLSI",sampleCells=30000,nRep=3)
# Add embeding
proj_subtype <- addUMAP(ArchRProj = proj_subtype, reducedDims = "IterativeLSI", name = "UMAP", nNeighbors = 10, minDist = 0.1,metric = "cosine")
proj_subtype <- addTSNE(ArchRProj = proj_subtype, reducedDims = "IterativeLSI", name = "TSNE",   perplexity = 30)
# Add markers
